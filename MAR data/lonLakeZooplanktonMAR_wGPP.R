...
#Using effect sizes of a model of hypothesized drivers, rather than running exhaustive search of potential drivers - this one uses GPP insetad of chlorophyll
...
#load covariate data
cov.mat<-read.csv('covariateData_wGPP_2011-2014.csv')
...
tsMat<-as.matrix(tot.data[,c(6:8,13)],ncol=4)
...
#make covariate matrix
covMat<-as.matrix(tot.data[,c(9:12,14)],ncol=5)
covMat<-covMat[1:nrow(covMat)-1,]
...
#Now try using only East Long data
